...
libh3_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "libh3.so.1")
...
class GeoJsonLite(Structure):
    ...
    _fields_ = [("geofence", Geofence), ("num_holes", c_int), ("holes", c_void_p)]
...
LinkedGeoCoord._fields_ = [("vertex", GeoCoord), ("next", POINTER(LinkedGeoCoord))]
...
def k_ring_distances(h3_address, ring_size):
    ...
    libh3.kRingDistances(string_to_h3(h3_address), ring_size, krings, distances)
    out = []
...
def _coord_array_to_geo_coord(coord_array, geo_json_conformant=False):
    ...
    return GeoCoord(degs_to_rads(mercator_lat(lat)), degs_to_rads(mercator_lng(lng)))
...
def _polygon_array_to_geofence(polygon_array, geo_json_conformant=False):
    ...
    for i in range(num_verts):
        geo_coord_array[i] = _coord_array_to_geo_coord(
            polygon_array[i], geo_json_conformant
        )
...
def _geo_json_to_geo_json_lite(geo_json, geo_json_conformant=False):
    ...
    geofence = _polygon_array_to_geofence(
        geo_json["coordinates"][0], geo_json_conformant
    )
    holes = None
    if num_holes > 0:
...
def hex_ring(h3_address, ring_size):
    ...
    if success != 0:
        raise Exception("Failed to get hexagon ring for pentagon {}".format(h3_address))
...
def compact(h3_addresses):
    ...
    if ret_val != 0:
        raise Exception("Failed to compact, malformed input data (duplicate hexagons?)")
...
def uncompact(h3_addresses, res):
    ...
    ret_val = libh3.uncompact(
        hex_set, num_hexagons, uncompacted_hex_set, max_uncompacted_num, res
    )
...
def hex_ranges(h3_address_list, ring_size):
    ...
    KringArray = H3Index * array_len
    # Initializes to zeroes by default, don't need to force
    hex_array = HexArray(*[string_to_h3(h3_address) for h3_address in h3_address_list])
    krings = KringArray()
    success = libh3.hexRanges(hex_array, num_hexagons, ring_size, krings)
    if success != 0:
        raise ValueError("One of the specified hexagon ranges contains a pentagon")
    out = {}
    for i in range(0, num_hexagons):
        ...
        for j in range(0, range_size):
            ...
            hex_range_list[ring_index].add(h3_to_string(krings[i * range_size + j]))
    return out
...
def hex_area(resolution, unit="km^2"):
    if unit == "km^2":
        ...
        raise ValueError('Provided unit not supported, must be "km^2" or "m^2"')
...
def get_h3_unidirectional_edge(a, b):
    edge_int = libh3.getH3UnidirectionalEdge(string_to_h3(a), string_to_h3(b))
    if edge_int == 0:
        raise ValueError("Provided H3Indexes are not hexagons or not neighbors")
...
def get_origin_h3_index_from_unidirectional_edge(h3_address):
    origin_int = libh3.getOriginH3IndexFromUnidirectionalEdge(string_to_h3(h3_address))
    if origin_int == 0:
...
def get_h3_indexes_from_unidirectional_edge(h3_address):
    ...
    libh3.getH3IndexesFromUnidirectionalEdge(string_to_h3(h3_address), index_array)
...
def get_h3_unidirectional_edges_from_hexagon(h3_address):
    ...
    index_array = IndexArray()
    libh3.getH3UnidirectionalEdgesFromHexagon(string_to_h3(h3_address), index_array)
    zero_count = 0
...
def get_h3_unidirectional_edge_boundary(h3_address, geo_json=False):
    ...
    geo_boundary = GeoBoundary()
    libh3.getH3UnidirectionalEdgeBoundary(string_to_h3(h3_address), byref(geo_boundary))
    out = []
    for i in range(geo_boundary.num_verts):
        out.append(
            [
...
def h3_line(h3_address_origin, h3_address_h3):
    ...
    index_array = IndexArray()
    ret_val = libh3.h3Line(
        string_to_h3(h3_address_origin), string_to_h3(h3_address_h3), index_array
    )
    if ret_val != 0:  # pragma: no cover
